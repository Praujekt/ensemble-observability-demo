...
# ── Azure Function App ─────────────────────────────────────────────────
resource "azurerm_service_plan" "ensemble" {
  name                = "ensemble-demo-plan"
  resource_group_name = azurerm_resource_group.ensemble.name
  location            = azurerm_resource_group.ensemble.location
  os_type             = "Linux"
  sku_name            = "Y1"
...
resource "azurerm_linux_function_app" "ensemble" {
  name                = "ensemble-serverless-demo"
  resource_group_name = azurerm_resource_group.ensemble.name
  location            = azurerm_resource_group.ensemble.location

  storage_account_name       = azurerm_storage_account.ensemble.name
  storage_account_access_key = azurerm_storage_account.ensemble.primary_access_key
  service_plan_id            = azurerm_service_plan.ensemble.id

  site_config {
    application_stack {
      python_version = "3.11"
    }
    application_insights_connection_string = azurerm_application_insights.ensemble.connection_string
    application_insights_key               = azurerm_application_insights.ensemble.instrumentation_key
  }

  app_settings = {
    APPINSIGHTS_INSTRUMENTATIONKEY = azurerm_application_insights.ensemble.instrumentation_key
